...
namespace StardewModdingAPI.Framework.ModLoading.Rewriters
{
    ...
    internal class ReplaceReferencesRewriter : BaseInstructionHandler
    {
        ...
        ****/
        ...
        {
            return this.MapFacade(typeof(TFromType).FullName!, typeof(TFacade));
        }
        ...
        public ReplaceReferencesRewriter MapFacade(string fromTypeName, Type toType)
        {
            // properties
            ...
            {
                string propertyType = RewriteHelper.GetFullCecilName(property.PropertyType);
            ...
            foreach (MethodInfo method in toType.GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.DeclaredOnly))
            {
                ...
                var paramTypes = method.GetParameters().Select(p => RewriteHelper.GetFullCecilName(p.ParameterType));
                string fromFullName = $"{RewriteHelper.GetFullCecilName(method.ReturnType)} {fromTypeName}::{method.Name}({string.Join(",", paramTypes)})";

                this.MapMember(fromFullName, method, "method");
            }

            // constructors
            foreach (ConstructorInfo constructor in toType.GetConstructors(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly))
            {
                var paramTypes = constructor.GetParameters().Select(p => RewriteHelper.GetFullCecilName(p.ParameterType));
                string fromFullName = $"System.Void {fromTypeName}::.ctor({string.Join(",", paramTypes)})";
